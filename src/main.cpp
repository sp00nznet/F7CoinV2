...
#if defined(NDEBUG)
# error "f7coin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb310fe3d27605eb29d914576ffdecd13c9a774745876dcdb9ca30b56cbe20ac3");
...
{
    // f7coin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // f7coin: 840k blocks in ~4 years
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // f7coin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb310fe3d27605eb29d914576ffdecd13c9a774745876dcdb9ca30b56cbe20ac3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        assert(block.hashMerkleRoot == uint256("0xb310fe3d27605eb29d914576ffdecd13c9a774745876dcdb9ca30b56cbe20ac3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // f7coin: increase each by adding 2 to bitcoin's value.
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "getheaders")
    {
        ...
        // we must use CBlocks, as CBlockHeaders won't include the 0f0 nTx count at the end
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("f7coinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("f7coinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static f7coinMiner(CWallet *pwallet)
{
    printf("f7coinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("f7coin-miner");
    ...
    try { loop {
        ...
        printf("Running f7coinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&f7coinMiner, pwallet));
}
